...
ensure_wallust_gtk_import() {
  # colors-gtk.css is regenerated by wallust every run; gtk.css itself is a
  # small tracked file that just imports it, so app-level GTK theme CSS
  # keeps working even if a user adds their own rules below the import.
  local gtk_dir="$1"
  local gtk_css="${XDG_CONFIG_HOME:-$HOME/.config}/$gtk_dir/gtk.css"
  local colors_file="${XDG_CONFIG_HOME:-$HOME/.config}/$gtk_dir/wallust/colors-gtk.css"
  [ -f "$colors_file" ] || return 0
  if [ -L "$gtk_css" ]; then
    # A theme-provided symlink (e.g. from .themes/<theme>/gtk.css) would have
    # our import overwrite the real theme file through the link - replace it
    # with our own tracked file instead.
    rm -f "$gtk_css"
  fi
  if [ ! -f "$gtk_css" ] || ! grep -q 'wallust/colors-gtk.css' "$gtk_css" 2>/dev/null; then
    mkdir -p "$(dirname "$gtk_css")"
    printf '@import url("wallust/colors-gtk.css");\n' >>"$gtk_css"
  fi
}
reload_gtk_apps() {
  # GTK3/4 apps only re-read gtk.css when notified of a theme change, not on
  # a plain file edit. Bounce gtk-theme through a throwaway value so running
  # apps (and Thunar's background instance) reload with the new colors.
  if command -v gsettings >/dev/null 2>&1; then
    local cur_theme
    cur_theme="$(gsettings get org.gnome.desktop.interface gtk-theme 2>/dev/null | sed "s/^'//;s/'$//")"
    if [ -n "$cur_theme" ]; then
      gsettings set org.gnome.desktop.interface gtk-theme "Adwaita" >/dev/null 2>&1 || true
      gsettings set org.gnome.desktop.interface gtk-theme "$cur_theme" >/dev/null 2>&1 || true
    fi
  fi
  # Thunar keeps a background D-Bus instance alive between windows; quitting it
  # is harmless (closes open windows) and makes the next launch pick up fresh
  # colors immediately instead of waiting on the gsettings bounce above.
  if command -v thunar >/dev/null 2>&1 && pgrep -x thunar >/dev/null 2>&1; then
...
# GTK targets are best-effort (see ensure_wallust_gtk_import/reload_gtk_apps
# below) so they're not in wallust_targets' hard failure check above.
mkdir -p "${XDG_CONFIG_HOME:-$HOME/.config}/gtk-3.0/wallust" "${XDG_CONFIG_HOME:-$HOME/.config}/gtk-4.0/wallust"
...
ensure_wallust_waybar_style
ensure_wallust_gtk_import "gtk-3.0"
ensure_wallust_gtk_import "gtk-4.0"
reload_gtk_apps
reload_running_cava_colors
